...
<head>
    ...
    <link rel="stylesheet" href="<?php echo asset('assets/fonts/line-awesome.min.css');?>">
...
<body>
    ...
    <section class="py-5 mt-5">
        <div class="container py-5">
            <h1 style="font-weight: bold;font-size: 30px;">Intelligence artificielle</h1>
            <hr style="margin-bottom: 40px;border: 2px solid var(--bs-black) ;">
            @foreach($liste_article as $article)
                <div class="d-flex align-items-center align-items-md-start align-items-xl-center">
                    <div>
                        <p style="color: #928c8c;"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-quote fs-1 text-warning flex-shrink-0">
                                <path d="M12 12a1 1 0 0 0 1-1V8.558a1 1 0 0 0-1-1h-1.388c0-.351.021-.703.062-1.054.062-.372.166-.703.31-.992.145-.29.331-.517.559-.683.227-.186.516-.279.868-.279V3c-.579 0-1.085.124-1.52.372a3.322 3.322 0 0 0-1.085.992 4.92 4.92 0 0 0-.62 1.458A7.712 7.712 0 0 0 9 7.558V11a1 1 0 0 0 1 1h2Zm-6 0a1 1 0 0 0 1-1V8.558a1 1 0 0 0-1-1H4.612c0-.351.021-.703.062-1.054.062-.372.166-.703.31-.992.145-.29.331-.517.559-.683.227-.186.516-.279.868-.279V3c-.579 0-1.085.124-1.52.372a3.322 3.322 0 0 0-1.085.992 4.92 4.92 0 0 0-.62 1.458A7.712 7.712 0 0 0 3 7.558V11a1 1 0 0 0 1 1h2Z"></path>
                            </svg>{{ $article->categorie }}</p>
                        <h4>{{ $article->titre }}</h4>
                        <p>{{ $article->resume }}</p>
                        <div class="d-flex">
                            <p>Publié le : {{ $article->getPublication()->publish_at }}&nbsp;</p>
                            <p><i class="la la-circle-thin"></i>&nbsp;</p>
                            <p class="fw-bold float-start me-2">{{ $article->getAuteur()->nom }} {{ $article->getAuteur()->prenom}}</p>
                        </div><a href="{{ url('/Details') }}/{{ $article->id }}">Plus d'informations&nbsp;<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-arrow-right">
                                <path fill-rule="evenodd" d="M1 8a.5.5 0 0 1 .5-.5h11.793l-3.147-3.146a.5.5 0 0 1 .708-.708l4 4a.5.5 0 0 1 0 .708l-4 4a.5.5 0 0 1-.708-.708L13.293 8.5H1.5A.5.5 0 0 1 1 8z"></path>
                            </svg></a>
                    </div>
                    <div><img src="<?php echo asset('{{ $article->image }}');?>" style="margin-left: 20px;width: 150px;"></div>
                </div>
                <hr>
            @endforeach
            <!-- PAGINATION -->
